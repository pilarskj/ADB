...
public class GammaLogLikelihoodJT {
    ...
    // BD solution for P0 (for initial guess, cf. Stadler, JTB 2010)
    public static double[] calcBDP0(double[] t, double C, double d, double rho) {
        ...
        double[] P0 = new double[t.length];
        IntStream.range(0, t.length)
                .parallel()
                .forEach(i -> {
                    P0[i] = (lambda + mu + c1 *
                            (Math.exp(-c1 * t[i]) * (1 - c2) - (1 + c2)) /
                            (Math.exp(-c1 * t[i]) * (1 - c2) + (1 + c2))) /
                            (2 * lambda);
                });
    ...
    // writer for testing
    private static void writeArray(double[] array, String fileName) throws IOException {
