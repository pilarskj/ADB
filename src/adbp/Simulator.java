...
public class Simulator {
    ...
    public Tree simulate(double origin, int originType, double[] a, double[] b, double[] d, double[][] Xsi_as, double[][] Xsi_s) {
        ...
        setType(root, originType); // type (trait)
        ...
        while (!events.isEmpty()) {
            ...
            int nodeType = getType(event); // type of current particle

            if (random.nextDouble() >= d[nodeType]) { // birth (current particle gives rise to two children)
    ...
    public double sampleLifetime(double a, double b) {
        GammaDistribution gammaDist = new GammaDistribution(b, a);
        return gammaDist.sample();
    }

    // Helper method to sample offspring types
    public int[] sampleChildrenTypes(int parentType, double[][] Xsi_as, double[][] Xsi_s) {
        ...
        for (int j = 0; j < ntypes; j++) {
            ...
            // assymmetric case
            cumulativeProb += Xsi_as[parentType][j];
            if (random.nextDouble() < cumulativeProb) {
    ...
    // Set the type (trait) of a node
    private void setType(Node node, int type) {
        node.setMetaData("type", type);  // Store the type as metadata
    }

    // Get the type (trait) of a node
    private int getType(Node node) {
        return (int) node.getMetaData("type");
    }
}
